...
def timeseries_dataset_from_array(
    ...
    if data_times is not None:
        ...
        for i in range(len(start_positions)):
            ...
            start_position_time = data_times[start_position]
            end_position_time = data_times[start_position + (sequence_length * sampling_rate)]
            if (start_position_time + (sequence_length * sampling_rate)) != end_position_time:
    ...
    if targets is not None and sample_weights is not None:
        indices = tf.data.Dataset.zip(
            (tf.data.Dataset.range(len(start_positions)), positions_ds)).map(
        ...
        sample_weights_ds = sequences_from_indices(
            sample_weights, indices, start_index, end_index)
        dataset = tf.data.Dataset.zip((dataset, target_ds, sample_weights_ds))
